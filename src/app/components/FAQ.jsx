...
export default function FAQ() {
  ...
  return (
    <section ref={sectionRef} id="faq" className="py-6 md:py-20 bg-gradient-to-br from-white via-[#fff0f3] to-[#ffe5ea] relative overflow-hidden">
      ...
      <div className="container mx-auto px-4">
        <div className="max-w-3xl mx-auto">
          ...
          <div className="space-y-2.5 md:space-y-5">
            {faqs.map((faq, index) => (
              <div key={index} className={`bg-white/80 border border-[#E60023]/10 rounded-2xl shadow-xl transition-all duration-300 ${isVisible ? `animate__animated animate__fadeInUp ${index === 0 ? 'animate__delay-0-5s' : index === 1 ? 'animate__delay-0-7s' : 'animate__delay-0-9s'}` : 'opacity-0'}`} style={{ backdropFilter: 'blur(6px)' }}>
                <button onClick={() => setOpenIndex(openIndex === index ? null : index)} className="w-full px-6 py-5 text-left flex items-center justify-between focus:outline-none group" aria-expanded={openIndex === index} aria-controls={`faq-answer-${index}`}>
                  <span className="font-semibold text-black text-lg group-hover:underline">{faq.question}</span>
          ...
          <div className="text-center mt-5 md:mt-10">
            <a href="#" className="inline-block text-[#E60023] font-semibold hover:underline transition-colors text-lg">
              See all FAQs →
            </a>
          </div>
        </div>
